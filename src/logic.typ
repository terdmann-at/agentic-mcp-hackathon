...
#let item-by-item(start: 1, mode: hide, body) = {
  ...
  // ADDED: Filter out items that have no content (fixes "ghost" slide issues)
  let real-items = children.filter(is-item).filter(it => it.body != [])
  one-by-one(start: start, mode: mode, ..real-items)
}
