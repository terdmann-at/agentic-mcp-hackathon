...
# Set HEADLESS=True for running in GitHub Codespaces or headless environments
...
#
# STRATEGY 1: "Think" Tool (Recommended for flexibility)
# Give the agent a `think` tool. If it wants to reason or plan, it MUST use this tool.
# This counts as a tool call, so the loop continues. Plain text is reserved ONLY for the
# final answer.
#
# STRATEGY 2: "Final Answer" Tool (Recommended for robustness)
# Give the agent a `final_answer` tool. Update `should_continue` to ONLY stop when this specific
# tool is called. This forces the agent to be explicit about maximizing supervision.
...
def main():
    ...
    for event in app.stream(
        inputs, stream_mode="values", config={"recursion_limit": 50}
    ):
